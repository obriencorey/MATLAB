...
lb = [Ha_lb, Hd_lb, Vdr_lb, V_lb, W_lb, F_lb];
ub = [Ha_ub, Hd_ub, Vdr_ub, V_ub, W_ub, F_ub];
...
function val = optfunc(x,E_targ,T_targ,E_gain,T_gain)  
    ...
    %descent time expression
    %equals drogue descent time plus main descent time
    T = (x(1)-x(2))/x(3) + x(2)/x(4);
