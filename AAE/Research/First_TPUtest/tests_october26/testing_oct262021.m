...
Fid1 = fopen('test2/test2voltage.txt','r');
tline = fgets(Fid1);
...
Fid2 = fopen('test2/test2strain.txt','r');
tline = fgets(Fid2);
...
subtitle("Monotonic Ramping, 10 wt. %, 1% engineering strain");
...
cutoffidx2 = length(microstrain)-length(normDeltaR);

if(cutoffidx2 > 0)
    straintruncated = microstrain(1:end-cutoffidx2);
else
    normDeltaR = normDeltaR(1:end-(-cutoffidx2));
    straintruncated = microstrain;
end
...
[M1,cutoffidx] = max(straintruncated);
...
P = polyfit(straintruncated(1:cutoffidx),normDeltaR(1:cutoffidx),1);
fprintf("loading gauge factor = %.3f\n",(100*(P(1))))
P = polyfit(straintruncated(cutoffidx:end),normDeltaR(cutoffidx:end),1);
fprintf("unloading gauge factor = %.3f\n",(100*(P(1))))
